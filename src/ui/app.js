...
require.config({
    ...
    paths: {
        ...
        'localStorage': '../libs/angularLocalStorage'
...
//@ifdef !production
define('manageApp.template', ['angular','localStorage'], function () {
...
define('manageApp', [
    ...
    return angular.module('manageApp', [
        ...
        $rootScope.$on('$routeChangeStart', function(evt, next, current,Auth) {
            Auth.isLoggedIn();
        });
    }]);
});
...
/*            $routeProvider
            ...
            $routeProvider
                .when("/:page*", {
                    templateUrl: function (param) {
                        var _url = param.page;
                        if(_url.indexOf('login')|| _url.indexOf('index')
                            || _url.indexOf('404')|| _url.indexOf('500')){
                            _url = (Config.viewsDir || '') + param.page;
                        }
                        delete param.page;
                        var _param = $.param(param);
                        return _param ? _url + "?" + _param : _url;
